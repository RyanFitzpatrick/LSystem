...
#define LS_NewMem(ptr, size) if ((ptr = malloc(size)) == NULL) goto FAIL
...
/* Param2 size: The size of the memory to point at, clean up with DiscardMem */
#define LS_ResizeMem(temp, ptr, size) { if ((temp = realloc(ptr, size)) == NULL) goto FAIL; ptr = temp; }

/* Releases a pointer using free */
/* Param1 ptr: The pointer to be released */
#define LS_DiscardMem(ptr) free(ptr)
